...
> **Environment note.** The isolation invariant relies on being able to spawn
> parallel, fresh-context subagents (the Agent/Task tool in Claude Code, or a
> workflow that fans out). In an environment without subagent spawning, this
> skill degrades to a wider single-context pass — still useful, but call out
> that the isolation guarantee isn't there.

## Source

This skill operationalises a written spec on divergent ideation — parallel
isolated Agent calls, a mechanical generator/critic split, and frame-based
branching. Background, an accompanying paper, evals, and an optional Node/TS
CLI (`npm install -g adhd-agent`) live at
https://github.com/UditAkhourii/adhd. The loop above runs inside Claude with
no install required.
