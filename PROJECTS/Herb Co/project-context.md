...
Each engineer keeps a local mirror of the iBiz source for static analysis and reference. Suggested layout (substitute your own path):

- **iBiz source mirror:** `<your-local-dev-path>/ibizfusion` (read-only mirror of Adrian's repo)
- **Active dev branch:** `<your-local-dev-path>/dev-main` (when working on changes alongside Adrian)
